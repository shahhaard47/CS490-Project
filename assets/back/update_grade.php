...
foreach($scoreInfo['scores'] as $arr)
{
  if(sizeOf($arr)>2)
  {
    $update_testCases = mysqli_query($conn, "UPDATE BETA_rawExamData SET testCasesPassFail='".implode(',',$arr["testCasesPassFail"])."',gradedComments='".implode('|',$arr["comments"])."',modResponse='".$arr['modResponse']."' WHERE userID='".$scoreInfo['userID']."' AND examID='".$scoreInfo['examID']."' AND questionID='".$arr["questionID"]."'");
  }
