...
//$scoreInfo = array('userID' => 'mscott', 'examID' => 6, 'scores' => [[1,18],[2,4]]); //TEST
...
{
  $update_gradesTable = mysqli_query($conn, "UPDATE BETA_grades SET examScore='".$totalExamScore."' WHERE userID='".$scoreInfo['userID']."' AND examID='".$scoreInfo['examID']."'");
}
else //meaning, a record for userID with examID does not exist and needs to be created
{
  $insert_gradesTable = mysqli_query($conn, "INSERT INTO BETA_grades (userID,examID,examScore) VALUES ('".$scoreInfo['userID']."','".$scoreInfo['examID']."','".$totalExamScore."')");
}
