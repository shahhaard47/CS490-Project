...
$requestInfo = json_decode($rawGradingRequest, true); //decode JSON request data for grading info
//$requestInfo = array('examID' => $data['examID'], 'userID' => $data['userID']); //store JSON request data for grading info



$result = mysqli_query($conn, "SELECT BETA_rawExamData.questionID,BETA_questionBank.points,BETA_questionBank.functionName,BETA_rawExamData.studentResponse,BETA_questionBank.testCases FROM BETA_rawExamData, BETA_questionBank WHERE BETA_rawExamData.questionID=BETA_questionBank.questionID AND BETA_rawExamData.examID='".$requestInfo['examID']."' AND BETA_rawExamData.userID='".$requestInfo['userID']."'");
